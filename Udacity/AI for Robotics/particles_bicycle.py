...
# with variance of self.steering_noise.   
    ...
    #           self.distance_noise
        ...
        xp=cx+sin(self.orientation+beta)*r
        yp=cy-cos(self.orientation+beta)*r
        thetap=(self.orientation+beta)%(2*pi)
        robotp.set(xp,yp,thetap)
        return robotp
...
#
    def sense(self): #do not change the name of this function
        self.bearing_noise = bearing_noise # Noise parameter: should be included in sense function.
        Z = []
        x=self.x
        y=self.y
        o=self.orientation+random.gauss(0,self.bearing_noise)
        # ENTER CODE HERE
        # HINT: You will probably need to use the function atan2()
        for landmark in landmarks:
            if(landmark[0]-y>0):
                Z.append((atan2(landmark[0]-y,landmark[1]-x))-o)
            else:
                if(landmark[1]-x<0):
                    Z.append(atan2(landmark[0]-y,landmark[1]-x)+(2*pi)-o)
                else:
                    Z.append((8*pi/4)+atan2(landmark[0]-y,landmark[1]-x)-o)
            ##print (atan2(landmark[0]+y,landmark[1]-x))
